 ...
 *    . message
 */
exports.Worker = function standaloneWorker$$Worker (dependancyResolver, options) {
  var hostname, port;
  ...
  async function beginSession () {
    // The following 10 or so lines are duplicated from the dcp repo in src/dcp-client/worker/evaluators/index.js
    const sandBoxDefaultSetUpFiles = [
      dcpConfig.worker.sandbox.bootstrapFilename,
      '/bravojs/bravo.js',
      '/sandbox-lib/env.js'
    ];
    const setUpFiles = options.setUpFiles || ['/sandbox-lib/sa-ww-simulation.js', '/sandbox-lib/event-loop-virtualization.js'].concat(sandBoxDefaultSetUpFiles);
    const promises = setUpFiles.map(dependancyResolver.fetchDependency);
    const codeFromFiles = await Promise.all(promises);
    const setUpCode = codeFromFiles.join('\n');

    var wrappedMessage = this.serialize(
      { type: 'initWorker', w: this.serial, ts: Date.now(), payload: setUpCode, origin: 'workerBootstrap' }
    ) + '\n'
    connected = true
