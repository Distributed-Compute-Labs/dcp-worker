...
const reportLogger = {
  init(worker, options) {
    ...
    // compute error file name
    this.errorFileName = path.parse(this.options.reportFile).name + '-err.tsv';
  },
  ...
  appendToReportFile(contents, error=false) {
    const fileName = error ? this.errorFileName : this.options.reportFile;
  ...
  sandbox$onError(sandbox, sandboxData, err) {
    this.appendToReportFile(`${Date.now()}\t${err.constructor.name}\t${err.errorCode}\t${err.message}\n`, error=true);
