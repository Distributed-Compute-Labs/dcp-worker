...
const reportLogger = {
  ...
  outputReportSummary() {
    const summary = [
      ...
      `Slices: (started/completed/failed)`,
      `${this.workerReport.slicesStarted}/${this.workerReport.slicesCompleted}/${this.workerReport.slicesFailed}`,
      '-',
      `Sandboxes: (initialized/terminated)`,
      `${this.workerReport.sandboxesInitialized}/${this.workerReport.sandboxesTerminated}`,
      '\n',
    ].join('\t');

    fs.writeFileSync(this.options.reportFile, summary, {
      flag: 'a',
    });
