...
function getLogger({ outputMode='detect' }) {
  ...
  switch (outputMode) {
    ...
    case 'report':
      return require('./worker-loggers/report');
    default:
...
Object.assign(exports, {
  ...
  startWorkerLogger(worker, options={}) {
    ...
    worker.supervisor.on('sandboxReady', (sandbox) => {
      ...
      sandbox.on('error', logger.sandbox$onError.bind(logger, sandbox, data));
      sandbox.on('sliceError', logger.sandbox$onSliceError.bind(logger, sandbox, data));
      sandbox.on('workerStop', logger.sandbox$onWorkerStop.bind(logger, sandbox, data));
      sandbox.on('terminate', logger.sandbox$onTerminate.bind(logger, sandbox, data));
    });
  }
});
