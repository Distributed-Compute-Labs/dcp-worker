...
self.wrapScriptLoading({ scriptName: 'webgpu-evaluator' }, (postMessage)=>{  
  if (typeof GPU !== 'undefined'){
    try{
      ...
      //typical webgpu usage involves navigator.gpu being able to requestAdapter without
      //passing a window. Mimic this behavior
      
      let _GPU = GPU;

      (function buildWebGPUPolyfills(GPU){
        ...
        let oldAdapter = GPU.requestAdapter;

        self.navigator = {};
        self.navigator.gpu = GPU;
        self.navigator.gpu.requestAdapter = async (options={}) => {
          console.log("REQUESTING ADAPTER");
          if (typeof options.window === 'undefined'){
            options.window = new WebGPUWindow({
              width: 640,
              height: 480,
              title: 'DCP-evaluator',
              visible: false
          ...
          const adapter         = await oldAdapter(options);
          const extensions      = adapter.extensions;
          const device          = await adapter.requestDevice(extensions)
