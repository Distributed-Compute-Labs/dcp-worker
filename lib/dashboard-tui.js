...
exports.init = function dashboard$$init(worker, options)
{
  ...
  function updateWorkerInfo(fetchInfo)
  {
    ...
    if (fetchInfo?.fetchStart)
      lastTDRequest = fetchInfo?.fetchStart;
    ...
    workerInfoPane.setContent([
      ...
      `   Last Task Request: ${new Date(lastTDRequest).toUTCString()}`,
      `      Slices in task: ${fetchInfo ? Object.values(fetchInfo.slices).reduce((acc, val) => acc + val, 0) : 0}`,
      `        Jobs in task: ${fetchInfo ? Object.keys(fetchInfo.jobs).length : 0}`,
    ].join('\n'));
    screen.render();
  }
