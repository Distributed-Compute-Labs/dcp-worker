...
exports.init = function dashboard$$init(worker, options)
{
  ...
  // Sup1 and Sup2 require different implementations for the progress bars to update properly
  if(worker.isSupervisor1)
  {
    replaceSandboxEvent('slice', function dashboard$$job(sandbox, sandboxData, ev) {
  ...
  {
    replaceSandboxEvent('job', function dashboard$$job(sandbox, sandboxData, ev) {
      sandboxData.progressData = {
        indeterminate: true,
        progress: 0,
        label: ev.name,
